...
def main():
    ...
    if args.dataset == "celebA":
        spurious_prompt = [
            "A photo of a male.",
            "A photo of a male celebrity.",
            "A photo of a man.",
            "A photo of a female.",
            "A photo of a female celebrity.",
            "A photo of a woman.",
        ]

        candidate_prompt = [
            "A photo of a male celebrity with dark hair.",
    ...
    if args.debias:
        ...
        # The minimizer of the calibration loss is
    ...
    for dix, split in enumerate(splits):
        dataset_predictions[split] = get_zeroshot_predictions(
            dataset_embeddings[split], text_embeddings, args, temperature=100.0
        )
    for ix, split in enumerate(splits):
        print(f"-" * len(split))
        print(f"Zero-shot {split} predictions")
        print(f"-" * len(split))
        evaluate_clip(dataset_predictions[split], dataloaders_base[ix], verbose=True)
